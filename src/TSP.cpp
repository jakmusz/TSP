...
std::vector<cost_t> CostMatrix::get_min_values_in_rows() const {
    ...
    for (const std::vector<cost_t>& row : matrix_) {
        min_values.push_back( *std::min_element(row.cbegin(), row.cend() ) );
    };
    return min_values;
}
...
std::vector<cost_t> CostMatrix::get_min_values_in_cols() const {
    std::vector<cost_t> min_values = {};
    CostMatrix matrix_transpose(matrix_);
    for (int i = 0; i < matrix_.size(); ++i) {
        for (int j = 0; j < matrix_[i].size(); ++j) {
            matrix_transpose[j][i] = matrix_[i][j];
        }
    }
    return matrix_transpose.get_min_values_in_rows();
}
